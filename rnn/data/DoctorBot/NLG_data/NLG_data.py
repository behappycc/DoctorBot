...
# select time
    ...
    create_json_list(json_list, select_time, "select(slot='time',time='",
                     "請選擇日期,", "請問您要哪天,")
    split_data(json_list, train_json, valid_json, test_json)
    json_list = []
# confirm intent
...
# End intent=5
    ...
    with open("train.json", "w") as data_write:
        print(train_json, file=data_write)
    data_write.close()
    with open("valid.json", "w") as data_write:
        print(valid_json, file=data_write)
    data_write.close()
    with open("test.json", "w") as data_write:
        print(test_json, file=data_write)
    data_write.close()
